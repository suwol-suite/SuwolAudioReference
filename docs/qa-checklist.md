...
- `npm.cmd run check:release-tag -- --tag=v0.1.3` passes before any tag is created.
...
- `package.json` version is `0.1.3`, `engines.node` is Node 24+, license is `Apache-2.0`, Windows packaging remains zip-first, and Linux release packaging includes AppImage/tar.gz.
...
- Open Settings > About and confirm version `0.1.3`, license `Apache-2.0`, release notes path, known issues path, and Windows distribution guide path.
...
- Extract `release\Suwol.Audio.Reference.0.1.3.Windows.x64.zip` to a writable folder and run the executable from the extracted `win-unpacked` folder.
...
- On a 0.1.2 Linux AppImage, confirm the 0.1.3 GitHub Release can be detected as an available update when the release assets are present.
