...
Use this guide for a human smoke pass before publishing a 0.1.6 build. The goal is to confirm that a real user can install, open, import, listen, organize, export, and recover from common errors without the app modifying original audio files.
...
Fill this in before publishing a 0.1.6 build:

- Tester:
- Date:
- Windows version:
- Artifact type: unpacked / extracted Windows zip / Linux AppImage / Linux zip / macOS DMG / macOS zip
- Artifact path:
- Fixture folder:
- Commit:
...
- Extract `release\SuwolAudioReference-0.1.6-win-x64.zip` to a normal writable folder before running the executable from the extracted `win-unpacked` folder.
- Use the GitHub Actions Linux artifact or GitHub Release Linux AppImage/zip for Linux smoke on a Linux test machine.
- On Linux AppImage, open Settings Updates and confirm manual check/download/install controls are visible.
- On Windows zip, Linux zip, or macOS assets, open Settings Updates and confirm manual GitHub Releases guidance is shown instead of auto-update controls.
...
5. Confirm version `0.1.6`, license `Apache-2.0`, release notes path, known issues path, and Windows distribution guide path are visible.
...
1. Confirm `docs/release-notes-0.1.6.md` matches the tested artifact.
