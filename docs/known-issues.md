...
This document lists known limitations and release notes for Suwol Audio Reference 0.1.6. These items should not block the MVP when the documented workaround is acceptable and the automated release gates pass.
...
- Symptom: 0.1.6 publishes Windows zip, Linux zip/AppImage, and macOS arm64 DMG/zip artifacts instead of installer-centered or app-store style packages.
...
- Future plan: Keep direct engine project mutation out of 0.1.6 unless a later phase explicitly scopes an integration.

## Historical Failed Tags

- Symptom: The `v0.1.2`, `v0.1.3`, and `v0.1.5` tags point at failed historical tag-workflow commits and are intentionally left untouched.
- Impact: Re-running those tag workflows would use their old commits and fail again.
- Temporary handling: Publish the corrected release as `v0.1.6` instead of moving or recreating old tags.
