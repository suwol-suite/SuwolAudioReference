...
Version 0.1.6 is the unified Windows, Linux, and macOS release workflow release. The existing `v0.1.1` release remains tied to its historical tag, and the failed `v0.1.2`, `v0.1.3`, and `v0.1.5` tags remain untouched because tag workflows run from the tagged commit. Version 0.1.6 keeps the current main-branch project workflow work and moves release publishing to the standardized `SuwolAudioReference-<version>-<platform>-<arch>` asset names. The main release path is:
...
- Release notes and known issues are documented in `docs/release-notes-0.1.6.md` and `docs/known-issues.md`.
...
The release tag must match `package.json` exactly. For example, `package.json` version `0.1.6` must use tag `v0.1.6`. CI runs `npm run check:release-tag` and fails the release build if the tag, package version, and `docs/release-notes-X.Y.Z.md` do not line up. Manual recovery through `workflow_dispatch` must provide a version tag such as `v0.1.6`; `main` is never accepted as a release tag.
...
The GPG step verifies the checksum file signature. The `shasum` step verifies that the downloaded release assets match the published SHA-256 hashes. The release also includes a versioned copy named `SuwolAudioReference-0.1.6-checksums.txt`.

Windows users:

1. Download `SuwolAudioReference-0.1.6-win-x64.zip`.
2. Extract it to a writable folder.
3. Run `Suwol Audio Reference.exe` from the extracted `win-unpacked` folder.

Linux users:

1. Download `SuwolAudioReference-0.1.6-linux-x64.AppImage` or `SuwolAudioReference-0.1.6-linux-x64.zip`.
...
- Windows zip artifact: `release\SuwolAudioReference-0.1.6-win-x64.zip`.
...
- [Release notes 0.1.6](./docs/release-notes-0.1.6.md)
